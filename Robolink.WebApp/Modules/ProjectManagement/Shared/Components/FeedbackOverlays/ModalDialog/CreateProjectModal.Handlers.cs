...
namespace Robolink.WebApp.Modules.ProjectManagement.Shared.Components.FeedbackOverlays.ModalDialog;
    ...
    private void HandleFieldChanged(string fieldName, object? value)
    {
        ...
        switch (fieldName)
        {
            ...
            case nameof(State.FormData.Priority):
                ...
                {
                    // Kiểm tra xem con số này có khớp với giá trị nào trong Enum ProjectPriority không
                    if (Enum.IsDefined(typeof(ProjectPriority), priority))
                    {
                        State.FormData.Priority = (ProjectPriority)priority;
                    }
                    else
                    {
                        Logger.LogWarning("Giá trị priority {Value} không hợp lệ", priority);
                    }
                }
                break;
        }
